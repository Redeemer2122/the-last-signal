...
const framePacks = await buildFramePacks(desktopFrames.length);
...
const desktopPackSize = framePacks.desktop.reduce((sum, pack) => sum + pack.byteLength, 0);
const mobilePackSize = framePacks.mobile.reduce((sum, pack) => sum + pack.byteLength, 0);
...
- Desktop delivery: ${framePacks.desktop.length} frame packs / ${formatBytes(desktopPackSize)}
- Mobile delivery: ${framePacks.mobile.length} frame packs / ${formatBytes(mobilePackSize)}
...
console.log(`Frame packs: desktop ${formatBytes(desktopPackSize)}; mobile ${formatBytes(mobilePackSize)}`);
